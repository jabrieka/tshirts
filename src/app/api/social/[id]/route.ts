...
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <g font-family="Impact, 'Arial Black', sans-serif" fill="${p.text}">
  ...
  <g transform="translate(60, 820)">
    ...
    <text x="28" y="50" font-family="Helvetica, Arial, sans-serif" font-size="22" fill="${p.background}" letter-spacing="4">STARTING AT</text>
    <text x="28" y="100" font-family="Impact, sans-serif" font-size="60" fill="${p.background}">$${design.basePrice.toFixed(0)}</text>
  </g>
  <g transform="translate(480, 820)">
    <rect x="0" y="0" width="540" height="120" rx="24" fill="${p.background}"/>
    <text x="28" y="50" font-family="Helvetica, Arial, sans-serif" font-size="22" fill="${p.secondary}" letter-spacing="4">ORDER BY</text>
    <text x="28" y="100" font-family="Impact, sans-serif" font-size="56" fill="${p.text}">${escape(deadline.toUpperCase())}</text>
  </g>
  <text x="${W/2}" y="${H-50}" text-anchor="middle" font-family="Helvetica, Arial, sans-serif" font-size="22" fill="${p.text}" letter-spacing="8">COSETTEPRODUCTIONS.COM</text>
</svg>`;
}
